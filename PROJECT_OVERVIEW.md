...
> - **2026-07-10 (no-crop hero)** — Hero carousel `<img>` switched from `object-fit: cover` to `object-fit: contain` (+ `object-position: center`) in `components/PropertyHeroCarousel.tsx`, so every uploaded photo is shown in full at its native aspect ratio rather than being cropped to the 72vh hero box. Any letterbox band that opens up is filled by the existing section background `#28231C`, so portrait/landscape photos both letterbox against the dark ink band already used by the scrim — no additional colour or component work needed. Plain `<img>` served as-is (no next/image), so images are already at source quality; quality/`sizes` tuning wasn't applicable. Verified on `lawn-house-close-1bed` and `marsh-wall-3bed-5958`.
> - **2026-07-10 (property-page map)** — Location map added to the property detail sidebar, directly under the Enquire card. Reuses the existing Leaflet + CARTO setup from `/rent` — same tile layer, gold-on-ink teardrop marker, `invalidateSize` / `ResizeObserver` blank-tile guards, popup styling — by extending `components/PropertyMap.tsx` with two optional props: `height` (CSS length override) and `initialZoom` (single-property zoom that bypasses `fitBounds` and calls `setView` at the requested level so the marker sits dead-centre in the narrow sidebar column). New client wrapper `components/PropertyLocationMap.tsx` handles the `next/dynamic({ ssr: false })` boundary (the property page is a server component so it can't call dynamic itself) + pins Leaflet's stylesheet to the route bundle, then renders a single-marker map at 400 px and the "Marker location is indicative to the street, not the door." disclaimer under it. Section heading uses the same `.eyebrow` style as Overview/Amenities. Only mounts when `property.coordinates` is set. Verified SSR + hydration on `lawn-house-close-1bed`, `marsh-wall-3bed-5958`, and `westferry-circus-4078`.
> - **2026-07-10 (map reliability)** — Fixed intermittent blank-tile rendering on `/rent` and property maps. **Root cause**: `PropertyMap`'s init `useEffect` had `[properties, onMarkerFocus, initialZoom]` as its dep list, but every caller was producing reference-unstable values on every render — `app/rent/page.tsx` computes `getLiveProperties().filter(...)` directly during render (no useMemo), `components/PropertyLocationMap.tsx` passes `properties={[property]}` (fresh literal), and `Reveal`'s `inView` state flip triggered a parent re-render right in the middle of the async `import('leaflet').then(...)` bootstrap. Effect A's cleanup ran before Leaflet had resolved, effect B started a second import, and the belt-and-braces `invalidateSize` timers from run A had already been cleared by the time the surviving map instance existed — so Leaflet read the container size a beat too early and half the tile grid stayed blank. CARTO's occasional flakes made it look intermittent. **Fix**: (a) content-based `propertiesKey` memo now drives the init effect's deps in place of the array reference, and `properties` + `onMarkerFocus` are read via refs inside the effect so callback identity changes never re-init; (b) invalidateSize is now bumped from FOUR independent triggers — ResizeObserver on the container, IntersectionObserver on visibility, `window.resize`, and a chained rAF/setTimeout(100/400/1200 ms) ladder; (c) added an OpenStreetMap fallback tile layer that swaps in automatically after three CARTO `tileerror` events without a matching `tileload`, so the map is never blank because of a provider outage; (d) added a shimmering skeleton overlay inside the map wrapper that stays up until the first `tileload` fires (or a 6-second safety timeout expires), so the user never sees a flash of empty container. Verified `/rent` and 2 property pages hard-refresh cleanly.
> - **2026-07-10 (prod-tile triage)** — Follow-up to the map reliability pass after a reported "tiles blank on Vercel prod, work on localhost" bug. **Diagnostic passes** (all green server-side): no `Content-Security-Policy` header returned by Vercel (`curl -I` on live routes shows only `strict-transport-security` + Vercel infrastructure headers); Leaflet's stylesheet is bundled into the prod CSS chunk (`/_next/static/chunks/0t48hzs_6fshe.css` contains the `.leaflet-tile-container` / `.leaflet-tile` rules intact); the deployed map JS chunk (`/_next/static/chunks/120w-eghc53sg.js`) is **byte-identical** to a fresh local `next build`, contains the CARTO URL, the OSM fallback URL, the `tileerror` handler, and the `vm-map-shimmer` skeleton, so the latest code IS on prod; both CARTO and OSM tile CDNs return 200 `image/png` to direct curl requests, including when `Referer: https://valeandmercer.co.uk/rent` and `Origin: https://valeandmercer.co.uk` are passed. Every server-side hypothesis (CSP, missing CSS, missed deploy) came back negative. Most likely remaining cause is a client-side block (browser extension / DNS filter / corporate proxy) silently suppressing `basemaps.cartocdn.com` requests without firing `image.onerror`, which is what Leaflet's `tileerror` event listens on. **Defensive changes**: (i) lowered `TILE_ERROR_THRESHOLD` from 3 → 2 so the OSM swap kicks in faster when errors DO fire; (ii) added a `PRIMARY_NO_LOAD_TIMEOUT_MS = 3500 ms` silent-block guard — if no `tileload` event has fired on the primary layer within that window, force-swap to OSM regardless of `tileerror`, covering the case where extensions swallow the error handler; (iii) added a `console.info('[vm-map] Primary tile provider fell back to OSM (…)')` breadcrumb so the DevTools console shows exactly which path triggered the swap, giving a diagnostic breadcrumb for future prod-only bug reports.
